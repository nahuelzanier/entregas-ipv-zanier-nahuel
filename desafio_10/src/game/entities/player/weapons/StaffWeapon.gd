...
onready var explosion_shockwave_sfx: AudioHandler = $ExplosionShockwaveSFX
	...
	if fx_anim.is_playing():
		...
		for input in input_map.keys():
			if Input.is_action_just_pressed(input) && character.mana >= mana_drain[input_map[input]]:
				var type: int = input_map[input]
				attack(type)
				if type == ATT_TYPE.SPECIAL:
					explosion_shockwave_sfx.play()
