...
export (float) var ACC_VAR: float = 1
export (float) var ACC_VAR_DEFAULT = 1
export (float) var H_SPEED_LIMIT: float = 600.0
export (int) var jump_speed: int = 500
export (int) var default_jump_speed = 500
export (float) var FRICTION_WEIGHT: float = 6.25
export (int) var default_gravity: int = 10
...
func _on_Acceleration_timeout():
	jump_speed = 57 * log(default_jump_speed + ((ACC_VAR_DEFAULT - ACC_VAR)*default_jump_speed))

	if Input.is_action_pressed("move_left") || Input.is_action_pressed("move_right"):
		if ACC_VAR > 0 && ACCELERATION < MAX_SPEED:
			ACCELERATION += ACC_VAR
			ACC_VAR -= 0.1
	else:
		ACC_VAR = ACC_VAR_DEFAULT
		ACCELERATION = 0
