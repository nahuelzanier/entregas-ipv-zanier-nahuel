...
func button_inactive_effect(button):
	CurrentMap.map_manager.call_deferred("replace", Tags.fl_empty, iso_x, iso_y)
	CurrentMap.map_manager.call_deferred("create_entity", Tags.et_trap_door, iso_x, iso_y)
